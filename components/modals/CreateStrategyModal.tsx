...
import { type StrategyStruct, type TokenDataStruct, type ReinvestStruct } from '@/lib/types/dca';
...
export function CreateStrategyModal({ isOpen, onClose, accountAddress }: CreateStrategyModalProps) {
  const { createStrategy } = useDCAAccount(accountAddress);
  const { address: userAddress } = useAccount();
  const publicClient = usePublicClient();
  const { refetch: refetchAccounts } = useAccountStats();
  ...
  const selectedTokenDecimals = formData.baseToken ? tokenList[formData.baseToken as TokenTickers]?.decimals : 18;
  const tokenApproval = useTokenApproval(
    formData.baseToken ? tokenList[formData.baseToken as TokenTickers]?.contractAddress.ETH_SEPOLIA || '' : '',
    selectedTokenDecimals
  );

  const createTokenData = (ticker: TokenTickers): TokenDataStruct => {
  ...
  const createReinvestData = (): ReinvestStruct => ({
  ...
  const handleCreateStrategy = async () => {
    ...
    try {
      // Validation
      if (!formData.baseToken || !formData.targetToken || !formData.amount || !formData.interval || !userAddress) {
      ...
      if (formData.fundAmount && parseFloat(formData.fundAmount) > 0) {
        ...
        try {
          hasAllowance = await tokenApproval.checkAllowance(
            userAddress,
            accountAddress,
            formData.fundAmount
          ).catch(() => false);
        ...
        if (!hasAllowance) {
          ...
          try {
            const approvalHash = await tokenApproval.approveToken(accountAddress, formData.fundAmount)
              .catch((error) => {
      ...
      const strategyData: StrategyStruct = {
      ...
      try {
        await refetchAccounts().catch(() => null);
      } catch (error) {
        console.warn('Account refresh warning:', error);
      }
  ...
  const getButtonText = () => {
    ...
    switch (step) {
      case 1: return 'Checking Approval...';
      case 2: return 'Approving Tokens...';
      case 3: return 'Creating Strategy...';
      ...
      <ModalContent>
        ...
        <ModalBody className="gap-4">
          ...
          <Select
            ...
            isDisabled={isProcessing}
          >
            <SelectItem key="3600" value="3600">Every Hour</SelectItem>
            <SelectItem key="86400" value="86400">Every Day</SelectItem>
            <SelectItem key="604800" value="604800">Every Week</SelectItem>
          </Select>
        </ModalBody>
        <ModalFooter>
          <Button 
